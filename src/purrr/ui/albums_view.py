...
class AlbumsView(Gtk.Box):
    """Grilla de álbumes ya escaneados: carátula, nombre, artista, año y cantidad de canciones.
    Activar un álbum (doble clic / Enter) lo pone a reproducir completo desde la primera pista."""

    __gsignals__ = {
        "album-activated": (GObject.SignalFlags.RUN_FIRST, None, (str, str)),  # album, display_artist
    }
    ...
    def _on_bind(self, _factory, list_item: Gtk.ListItem) -> None:
        ...
        texture = None
        if album.art_path and Path(album.art_path).exists():
            texture = load_texture_at_size(album.art_path, _ART_SIZE)
    ...
    def _on_activate(self, _view, position: int) -> None:
        ...
        self.emit("album-activated", album.album, album.display_artist)
